...
float ssao(vec2 base_seed)
{
    ...
    float current_depth = texture2D(tex_normal, tcoord).a;
    vec3 current_pos = vec3(tcoord.xy, current_depth);
    vec3 current_normal = texture2D(tex_normal, tcoord).xyz;
    ...
    for (int i=0; i<samples; i++)
    {
        ...
        vec4 occluder_fragment = texture2D(tex_normal, se.xy);
    ...
    return 1.0 - 1.1 * sum * inv_samples;
}
